...
function TippsPage({ player, phaseId }) {
  ...
  async function updateKOPrognosisDB(playerId, phId, koData, currentTips, context) {
    ...
    const finalRecord = {
      ...
      reached_2:  (currentId >= 5) ? [] : r2.flatMap((_, i) => [getTeamForPrognosis(3, i, "A"), getTeamForPrognosis(3, i, "B")]).filter(Boolean),
      ...
      drop_out_4:  (currentId >= 5) ? [] : r4.map((_, i) => getProgLoser(2, i)).filter(Boolean),
